...
class TrackMyFiles(MDApp):
    ...
    def show_modal(self, instance):
        if not self.modal:
            self.modal = ModalView(size_hint=(.5, .5), auto_dismiss=True, background='', background_color=[0, 0, 0, 0])
            self.moda.add_widget(Image(source='./assets/gif.gif', anim_delay=0.03))  # Load and play the GIF
            self.modal.bind(on_pre_open=self.modal_open, on_pre_dismiss=self.modal_dismiss)
            self.modal.open()
            self.modal.opacity = 0

    def modal_open(self, instance):
        print('Modal is open 90')
        # instance.opacity = 0
        print(instance.opacity)
        Animation(opacity=1, duration=.5).start(instance)
